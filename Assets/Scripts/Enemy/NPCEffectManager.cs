...
public class NPCEffectManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        playerHealth = GetComponent<NPCHealth>();
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Fire"))
        {
            if (!playerHealth.isInmune)
            {
                ...
                //knockBack
                player.GetComponent<FindTarget>().kBForce = playerHitting.GetComponent<PlayerMovement>().kBForce;
                player.GetComponent<FindTarget>().kBCounter = player.GetComponent<FindTarget>().kBTotalTime;
