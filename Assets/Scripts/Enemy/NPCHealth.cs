...
public class NPCHealth : MonoBehaviour
{
    ...
    private void Respawn(GameObject player)
    {
        ...
        health = 130;
        healthBar.SetHealth(health);
        player.GetComponent<NPCPowerUpManager>().ResetStats();
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Damage"))
        {
            if (!isInmune)
            {
                ...
                //int damage = collision.gameObject.GetComponentInParent<NPCAttack>().damage;
                //knockBack

                if (!playerHitting.GetComponent<PlayerMovement>())
                {
                    player.GetComponent<FindTarget>().KnockBack(playerHitting.GetComponent<FindTarget>().kBForce, playerHitting.GetComponent<FindTarget>().kBTotalTime);
                    player.GetComponent<FindTarget>().kBCounter = playerHitting.GetComponent<FindTarget>().kBTotalTime;
                }

                player.GetComponent<FindTarget>().KnockBack(playerHitting.GetComponent<PlayerMovement>().kBForce, playerHitting.GetComponent<PlayerMovement>().kBTotalTime);
                player.GetComponent<FindTarget>().kBCounter = playerHitting.GetComponent<PlayerMovement>().kBTotalTime;

                //OJO AQUI ------------------------------------------------------------------------------------------------------------------
                if (collision.transform.position.x >= player.transform.position.x)
                {
                    player.GetComponent<FindTarget>().knockFromRight = true;
                }
                if (collision.transform.position.x <= player.transform.position.x)
                {
                    player.GetComponent<FindTarget>().knockFromRight = false;
                }
                //RecibeDano
        ...
        if (collision.CompareTag("Power"))
        {
            if (powerCountDown <= 0)
            {
                ...
                player.GetComponent<NPCPowerUpManager>().GetPower(iD);
